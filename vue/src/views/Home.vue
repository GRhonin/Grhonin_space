<template>
	<div class="content">
		...
		<div class="empty"></div>
		<!--两个轮播图-->
		<swiper  :options="swiperOption" v-for="item in groups" :key="item.id">
			<swiper-slide  v-for="item2 in getItems(item)" :key="item2.id">
				<img class="swiperImg" :src="item2.avatar" alt="暂无相关图片">
			</swiper-slide>
		</swiper>
		<!--下面宣传列表-->
		<!--<button @click="test1" class="btn">test1</button>-->
		<div class="detail" v-for="i in list2" :key="i.id">
			<h2 v-text="i.h2"></h2>
			<div class="title" v-text="i.h1"></div>
			<div class="line"></div>
			<div class="version" v-text="i.h3"></div>
			<img :src="i.avatar" alt="暂无相关图片">
